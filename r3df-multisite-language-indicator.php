...
/*  Copyright 2017 R-Cubed Design Forge
...
class R3DF_Multisite_Language_Indicator {
	...
	function user_profile_settings( $user ) {
		...
		<table class="form-table">
			...
			<tr valign="top">
				...
				<td>
					<label for="enable_locale_flags[before]"><input type="checkbox" id="enable_locale_flags[before]"
																	name="r3df_multisite_language_indicator[enable_locale_flags][before]"<?php echo checked( ! empty( $options['enable_locale_flags']['before'] ), true, false ); ?>
																	value="true">
						<?php _e( 'Country flags - before site name', 'r3df-multisite-language-indicator' ); ?></label>
					<br>
					<label for="enable_locale_abbreviations[before]"><input type="checkbox"
																			id="enable_locale_abbreviations[before]"
																			name="r3df_multisite_language_indicator[enable_locale_abbreviations][before]"<?php echo checked( ! empty( $options['enable_locale_abbreviations']['before'] ), true, false ); ?>
																			value="true">
						<?php _e( 'Locale code - before site name', 'r3df-multisite-language-indicator' ); ?></label>
					<br>
					<label for="enable_locale_abbreviations[after]"><input type="checkbox"
																		   id="enable_locale_abbreviations[after]"
																		   name="r3df_multisite_language_indicator[enable_locale_abbreviations][after]"<?php echo checked( ! empty( $options['enable_locale_abbreviations']['after'] ), true, false ); ?>
																		   value="true">
						<?php _e( 'Locale code - after site name', 'r3df-multisite-language-indicator' ); ?></label>
					<br>
					<label for="display_language[before]"><input type="checkbox" id="display_language[before]"
																 name="r3df_multisite_language_indicator[display_language][before]"<?php echo checked( ! empty( $options['display_language']['before'] ), true, false ) ?>
																 value="true">
						<?php _e( 'Site language - before site name', 'r3df-multisite-language-indicator' ); ?></label>
					<br>
					<label for="display_language[after]"><input type="checkbox" id="display_language[after]"
																name="r3df_multisite_language_indicator[display_language][after]"<?php echo checked( ! empty( $options['display_language']['after'] ), true, false ); ?>
																value="true">
					...
					<table class="mli-site-table">
						...
						foreach ( get_sites() as $site ) {
							...
							<tr valign="top">
								...
								<td>
									<select id="site_flag[<?php echo $site -> blog_id ?>]"
											name="r3df_multisite_language_indicator[site_flag][<?php echo $site -> blog_id ?>]">
										<?php
	...
	function help_global_options() {
		...
		<p class="r3df-help">
			<a href="http://wordpress.org/extend/plugins/r3df-multisite-language-indicator/"
			   target="_blank"><?php echo __( 'Plugin Directory', 'r3df-multisite-language-indicator' ) ?></a> |
			<a href="http://wordpress.org/extend/plugins/r3df-multisite-language-indicator/changelog/"
			   target="_blank"><?php echo __( 'Change Logs', 'r3df-multisite-language-indicator' ) ?></a>
			<span class="alignright">&copy; 2015 <?php echo __( 'by', 'r3df-multisite-language-indicator' ) ?> <a
						href="http://r3df.com/" target="_blank">R3DF</a></span>
		</p>
		<?php
	}
	...
	function get_locale_language_name( $locale, $mode = 'native' ) {
		if ( ! in_array( $mode, array( 'english', 'native', 'localized' ), true ) ) {
			$mode = 'native';
		}
		$localized = false;
		if ( 'localized' === $mode ) {
